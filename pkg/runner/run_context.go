...
func (rc *RunContext) options(ctx context.Context) string {
	c := rc.resolveJobContainer(ctx)
	if c != nil {
		// Merge job container options with the CLI --container-options flag; the CLI value is appended last so docker's single flag parse lets it win scalar conflicts (repeatable flags accumulate). c.Options is already interpolated by resolveJobContainer.
		return strings.TrimSpace(c.Options + " " + rc.Config.ContainerOptions)
	}
